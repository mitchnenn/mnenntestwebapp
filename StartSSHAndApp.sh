...
dotnet /app/MnennTestWebApp.dll </dev/null &>/dev/null &status=$?
if [[ ${status} -ne 0 ]]; then
...
while sleep 5; do
  ps aux | grep dotnet | grep -q -v grep PROCESS_1_STATUS=$?
  ps aux | grep sshd | grep -q -v grep PROCESS_2_STATUS=$?
